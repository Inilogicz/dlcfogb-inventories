...
import { OfferingSubmission, ServiceType, Center } from "@/types/database"
import { Loader2, PhilippinePeso as Naira, Calendar, Building2 } from "lucide-react"

export default function OfferingList({ role, centerId }: { role: string, centerId?: string | null }) {
    const [submissions, setSubmissions] = useState<OfferingSubmission[]>([])
    ...
    if (submissions.length === 0) {
        return (
            <div className="bg-white p-8 rounded-xl border border-gray-100 text-center space-y-3">
                <Naira className="w-12 h-12 text-gray-300 mx-auto" />
                <h3 className="text-lg font-semibold text-gray-900">No offering records found</h3>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-100 font-bold text-gray-600">
                            ...
                            <th className="px-4 py-3 text-sm text-right">80% Remit</th>
                            <th className="px-4 py-3 text-sm text-right">20% Local</th>
                            <th className="px-4 py-3 text-sm text-right">Total (100%)</th>
                        </tr>
                    </thead>
                            ...
                            <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-4 py-3 text-right">
                                    <span className="text-sm font-medium text-green-600">
                                        ₦{(sub.amount_80 || sub.amount_100 * 0.8).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                    </span>
                                </td>
                                <td className="px-4 py-3 text-right">
                                    <span className="text-sm font-medium text-blue-600">
                                        ₦{(sub.amount_20 || sub.amount_100 * 0.2).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                    </span>
                                </td>
                                <td className="px-4 py-3 text-right">
                                    <span className="text-sm font-bold text-brand-blue">
                                        ₦{Number(sub.amount_100).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                    </span>
                                </td>
                            </tr>
                        ))}
