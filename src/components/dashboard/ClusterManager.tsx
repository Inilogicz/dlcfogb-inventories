...
// ----- Create Modal -----
function CreateClusterModal({ onClose, onCreated }: { onClose: () => void, onCreated: () => void }) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        const { data: cluster, error: clusterError } = await supabase
            .from('clusters').insert({ name }).select().single()

        if (clusterError || !cluster) {
            setError(clusterError?.message || "Failed to create cluster")
            setLoading(false)
            return
        }
        ...
        if (!res.ok) {
            ...
            setError(`Cluster created but admin setup failed: ${err.error}`)
        } else {
            onCreated()
            onClose()
        }
                ...
                <form onSubmit={handleSubmit} className="p-7 space-y-5">
                    {step === 1 ? (
                        <div className="space-y-4">
                            ...
                            <p className="text-xs text-gray-400 bg-indigo-50 rounded-xl px-4 py-3 border border-indigo-100">
                                💡 A cluster represents a geographic or administrative zone that groups multiple fellowship centers together.
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-4">
...
export default function ClusterManager() {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Modals */}
            {creating && <CreateClusterModal onClose={() => setCreating(false)} onCreated={handleCreated} />}
